...
TEST_CASE( "Testing the insert() function and verifying with the bool results and size function", "[insert(), size()]" ) {
    ...
    SECTION("Check size, clear, then verify load ==0 and is_empty == true") {
        REQUIRE( map1.size() == 4); 
...
TEST_CASE("Testing insert to maximum capacity for hashmap_oa", "[insert(), load(), ]") {
    ...
    bool ins45 = map1.insert(47,'c');
        ...
        SECTION("Search() - verify that value gets replace on successful search.") {
            //bool ins45 = map1.insert(47,'c');
            char val = 'd';
    ...
    bool insSame_Key = map1.insert(47,'e');       //TODO
        ...
        SECTION("Verify same key insert is functional and search replaces value") {
            char val = 'x';
            REQUIRE(map1.search(47, val));
            REQUIRE(val == 'e');
        }
