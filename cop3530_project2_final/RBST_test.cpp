...
inline bool cstring_equals( char const* s1, char const* s2 ) {
    for(; *s1 == *s2; ++s1, ++s2)
        if(*s1 == 0)
            return true;
    return false;
...
inline bool cstring_lt( char const* s1, char const* s2 ) {
    for(; *s1 == *s2; ++s1, ++s2)
        if(*s1 == 0)
            return true;
    return false;
...
static RBST<char const*, char, true, bool(*)(char const*, char const*), 
        cstring_equals, true, bool(*)(char const*, char const*), cstring_lt> map(50);
